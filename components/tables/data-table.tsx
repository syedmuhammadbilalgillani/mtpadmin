...
export type DataTableColumn<Row> = {
  ...
  cell?: (ctx: { row: Row; value: unknown; rowIndex: number }) => React.ReactNode
  ...
  statusVariantMap?: Record<string, React.ComponentProps<typeof Badge>["variant"]>
...
function breakpointHiddenClass(hideBelow: DataTableColumn<unknown>["hideBelow"] | undefined) {
  if (!hideBelow) return ""
...
function formatCellValue<Row>(
  ...
  switch (type) {
    case "number": {
      ...
      return new Intl.NumberFormat(undefined, column.format as Intl.NumberFormatOptions).format(num)
    }
    case "currency": {
      const num = typeof value === "number" ? value : Number(value)
      if (Number.isNaN(num)) return String(value)
    ...
    case "datetime": {
      ...
      const format: Intl.DateTimeFormatOptions =
        type === "date"
          ? { year: "numeric", month: "short", day: "2-digit", ...(column.format as Intl.DateTimeFormatOptions) }
          : { year: "numeric", month: "short", day: "2-digit", hour: "2-digit", minute: "2-digit", ...(column.format as Intl.DateTimeFormatOptions) }
      return new Intl.DateTimeFormat(undefined, format).format(date)
    }
    case "boolean": {
      const bool = Boolean(value)
      return (
        <span className={cn("text-sm", bool ? "text-foreground" : "text-muted-foreground")}>
          {bool ? column.trueLabel ?? "Yes" : column.falseLabel ?? "No"}
        </span>
      )
    }
    case "badge":
      return <Badge variant={column.badgeVariant ?? "secondary"}>{String(value)}</Badge>
    case "status": {
    ...
    case "tags": {
      const tags = Array.isArray(value) ? value : String(value).split(column.tagsSeparator ?? ",")
    ...
    case "image": {
      ...
      return (
        <div className="flex items-center gap-2">
          ...
          <img src={src} alt="" className="h-8 w-8 rounded-md border object-cover" />
...
export function DataTable<Row>({
  ...
  const selectedKeySet = React.useMemo(() => new Set(resolvedSelectedRowKeys), [resolvedSelectedRowKeys])
  ...
  const setSelectedKeys = React.useCallback(
    ...
    [data, getRowId, onSelectedRowKeysChange, onSelectedRowsChange, selectedRowKeys]
  )
  ...
  const totalPages = enablePagination ? Math.max(1, Math.ceil(data.length / currentPageSize)) : 1
  const safePage = Math.min(Math.max(1, currentPage), totalPages)
  const startIndex = enablePagination ? (safePage - 1) * currentPageSize : 0
  const endIndex = enablePagination ? startIndex + currentPageSize : data.length
  const pageRows = enablePagination ? data.slice(startIndex, endIndex) : data
  const pageRowKeys = React.useMemo(
    () =>
      pageRows.map((row, index) =>
        getRowId ? getRowId(row, startIndex + index) : String(startIndex + index)
      ),
    [getRowId, pageRows, startIndex]
  )
  const selectedCount = resolvedSelectedRowKeys.length
  const pageSelectedCount = pageRowKeys.filter((key) => selectedKeySet.has(key)).length
  const isPageAllSelected = pageRows.length > 0 && pageSelectedCount === pageRows.length
  const isPagePartialSelected = pageSelectedCount > 0 && pageSelectedCount < pageRows.length

  const sortedColumns = React.useMemo(() => {
    return [...columns].sort((a, b) => (a.priority ?? 999) - (b.priority ?? 999))
  }, [columns])
  ...
  return (
    <div className={cn("space-y-3", className)}>
      <Table className={tableClassName}>
        {caption ? <caption className="mt-2 text-sm text-muted-foreground">{caption}</caption> : null}
        <TableHeader>
          <TableRow>
            {enableRowSelection ? (
              <TableHead
                ...
                <input
                  ...
                  onChange={(event) => {
                    if (event.target.checked) {
                      ...
                      const filtered = resolvedSelectedRowKeys.filter((key) => !pageKeySet.has(key))
              ...
              return (
                <React.Fragment key={rowId}>
                  ...
                  >
                    {enableRowSelection ? (
                      <TableCell>
                        <input
                          ...
                          onChange={(event) => {
                            if (event.target.checked) {
                              setSelectedKeys(Array.from(new Set([...resolvedSelectedRowKeys, rowId])))
                            } else {
                              setSelectedKeys(
                                resolvedSelectedRowKeys.filter((key) => key !== rowId)
                              )
                            }
                          }}
                          className="h-4 w-4 rounded border border-input accent-primary"
                    ...
                    {columns.map((column) => {
                      const value = column.accessor ? column.accessor(row) : (row as never)[column.id]
                        ...
                        <div className="grid gap-2 rounded-md bg-muted/40 p-3">
                          {mobileDetailsColumns.map((column) => {
                            ...
                            const content = column.cell
                              ...
                              : formatCellValue(column, value, row)
                            return (
                              <div key={column.id} className="grid grid-cols-3 gap-3">
                                <div className="col-span-1 text-xs font-medium text-muted-foreground">
                                  {column.header}
                                </div>
                                <div className="col-span-2 text-sm">{content}</div>
                              </div>
                            )
                          })}
                        </div>
