...
type FieldDependencyOperator = "equals" | "notEquals" | "in" | "notIn" | "truthy" | "falsy"
...
type InputField<TFieldValues extends FieldValues> = DynamicFieldBase<TFieldValues> & {
  type: "input"
  inputType?: React.InputHTMLAttributes<HTMLInputElement>["type"]
  inputProps?: Omit<React.ComponentProps<"input">, "name" | "type" | "disabled" | "required">
}

type TextareaField<TFieldValues extends FieldValues> = DynamicFieldBase<TFieldValues> & {
  type: "textarea"
  textareaProps?: Omit<React.ComponentProps<"textarea">, "name" | "disabled" | "required">
}

type SelectField<TFieldValues extends FieldValues> = DynamicFieldBase<TFieldValues> & {
  type: "select"
...
type RadioField<TFieldValues extends FieldValues> = DynamicFieldBase<TFieldValues> & {
  type: "radio"
  options: DynamicOption[]
  radioGroupClassName?: string
}

type SwitchField<TFieldValues extends FieldValues> = DynamicFieldBase<TFieldValues> & {
  type: "switch"
}

type CheckboxField<TFieldValues extends FieldValues> = DynamicFieldBase<TFieldValues> & {
  type: "checkbox"
}

type CheckboxGroupField<TFieldValues extends FieldValues> = DynamicFieldBase<TFieldValues> & {
  type: "checkbox-group"
  options: DynamicOption[]
...
function FieldShell({
  ...
  return (
    <div className={cn("space-y-2", className)}>
      ...
      {error ? <p className={cn("text-xs text-destructive", errorClassName)}>{error}</p> : null}
    </div>
  )
}
          ...
          <Controller
            ...
            render={({ field: controllerField }) => (
              <Select
                ...
                disabled={field.disabled}
              >
                <SelectTrigger id={id} className={cn("w-full", field.selectProps?.triggerClassName)}>
            ...
            render={({ field: controllerField }) => (
              ...
              >
                {field.options.map((option) => (
                  ...
                  >
                    <RadioGroupItem value={option.value} disabled={option.disabled || field.disabled} />
            ...
            render={({ field: controllerField }) => {
              ...
              return (
                <div
                  className={cn(
                    "gap-2",
                    field.direction === "row" ? "flex flex-wrap items-center" : "grid"
                  )}
                >
                  {field.options.map((option) => {
    ...
    <form onSubmit={form.handleSubmit(handleSubmit)} className={cn("space-y-6", className)}>
      {sections.length ? (
        sections.map((section) => (
          <section key={section.id} className={cn("space-y-4", section.className)}>
            ...
            <div className={cn("grid grid-cols-1 gap-4 md:grid-cols-12", formGridClassName)}>
              {section.fields
                .filter((field) => shouldShowField(field, currentValues))
                .map((field) => (
                  <div key={field.name} className={cn(getGridSpanClass(field.colSpan), field.className)}>
